...
export default function Navigation({ title = 'Home' }: { title?: string }) {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-40 flex h-14 items-center border-b border-concrete bg-card">
        <div className="relative mx-auto flex h-full w-full max-w-[600px] items-center justify-center px-4 md:ml-[80px] md:mr-auto">
                  ...
                  <Menu.Items className="menu-panel absolute right-0 z-20 mt-2 w-44 origin-top-right py-2 focus:outline-none">
                    <Menu.Item>
                      {({ active }) => (
                        <Link
                          href="/profile"
                          className={`block px-4 py-2 text-[13px] font-semibold ${active ? 'bg-hover-mist' : 'text-ink'}`}
                        >
                          Your Profile
                        </Link>
                      )}
                    ...
                    <Menu.Item>
                      {({ active }) => (
                        <button
                          ...
                          className={`block w-full px-4 py-2 text-left text-[13px] font-semibold ${active ? 'bg-hover-mist' : 'text-ink'}`}
                        >
                          Sign out
                        </button>
                      )}
      ...
      {user && (
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-20 flex-col items-center border-r border-concrete bg-card pt-4 md:flex">
          <Link href="/feed" className="icon-btn mb-6 text-[15px] font-semibold" aria-label="Connecto">
            C
          </Link>
          <nav className="flex flex-1 flex-col items-center gap-6">
            {items.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`relative flex h-12 w-full items-center justify-center ${active ? 'before:absolute before:left-0 before:h-6 before:w-1 before:bg-ink' : ''}`}
                >
            ...
            <button type="button" aria-label="Search" className="icon-btn" onClick={() => setSearchOpen(true)}>
              <MagnifyingGlassIcon className="h-6 w-6" strokeWidth={1.6} />
            </button>
            <button type="button" aria-label="Compose" className="icon-btn compose-btn" onClick={goCompose}>
      ...
      {user && (
        <nav className="fixed inset-x-0 bottom-0 z-40 flex h-14 items-center justify-around border-t border-concrete bg-card md:hidden">
        ...
        <div className="search-overlay" onClick={() => setSearchOpen(false)}>
          <div className="search-panel" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <p className="text-[15px] font-semibold">Search</p>
              <button type="button" className="text-[13px] font-semibold text-graphite" onClick={() => setSearchOpen(false)}>
                Close
              </button>
            </div>
            <UserSearch />
